...
func NewChannelPool() *ChannelPool {
	return &ChannelPool{pending: make(map[uint64](chan error))}
}

// GetNewChannel allocates a new channel associated with an ID
func (cp *ChannelPool) GetNewChannel() (uint64, chan error) {
	ch := make(chan error)
...
func (cp *ChannelPool) Remove(channelID uint64) chan error {
	cp.mutex.Lock()
	defer cp.mutex.Unlock()
